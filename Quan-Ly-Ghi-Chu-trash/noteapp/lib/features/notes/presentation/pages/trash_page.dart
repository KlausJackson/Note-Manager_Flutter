            ...
            children: [
              ...
              Expanded(
                child: Builder(builder: (_) {
                  ...
                  if (filtered.isEmpty) {
                    return Center(key: const ValueKey('empty'), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.delete_outline, size: 48, color: Colors.grey), const SizedBox(height: 16), const Text('Không có ghi chú trong thùng rác')],),);
                  }
                            ...
                            if (direction == DismissDirection.startToEnd) {
                              ...
                              final confirm = await showDialog<bool>(
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  content: const Text('Bạn có muốn xõa vĩnh viễn ghi chú này không?'),
                                  actions: [
                                    TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Có')),
                                    TextButton(onPressed: () => Navigator.of(ctx).pop(true), child: const Text('Không')),
                                  ],
                                ),
                              );
                              if (confirm == true) {
                                try {
                                  ...
                                  if (mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(content: Text('Không thể xóa ghi chú: $e')),
                                    );
                                  }
                                  return false;
                                }
